...
def copyFile(src, dst):
    ensureDir(os.path.dirname(dst) or ".")
    with open(src, "rb") as fIn:
        with open(dst, "wb") as fOut:
            shutil.copyfileobj(fIn, fOut)

def signApk(fnIn, fnOut, keystore, keyAlias, keystorePass, zipalign=None, apksigner=None):
    zipAlignLoc = zipalign or which("zipalign")
    if not zipAlignLoc:
    ...
    print("Aligning...")
    if subprocess.run([zipAlignLoc, "-p", "-v", "4", fnIn, fnOut]).returncode != 0:
    ...
    apksignerLoc = apksigner or which("apksigner")
    if not apksignerLoc:
...
def patchApk(fnIn, fnOut, keystore, keyAlias, keystorePass, zipalign=None, apksigner=None):
    ...
    inZip.close()
    outZip.close()
    signApk(outZip.filename, fnOut, keystore, keyAlias, keystorePass, zipalign=zipalign, apksigner=apksigner)

    print("Removing temporary file...")
    os.remove(outZip.filename)

def detectBaseApk(splitDir, explicitBaseApk=None):
    if explicitBaseApk:
        return explicitBaseApk

    manifestPath = os.path.join(splitDir, "manifest.json")
    if os.path.exists(manifestPath):
        with open(manifestPath, "r", encoding="utf-8") as f:
            manifest = json.load(f)
        for split in manifest.get("split_apks", []):
            if split.get("id") == "base" and split.get("file"):
                return split["file"]

    apks = sorted([name for name in os.listdir(splitDir) if name.lower().endswith(".apk")])
    if len(apks) == 1:
        return apks[0]
    raise Exception("Could not determine base APK automatically. Pass --base-apk.")

def patchSplitDirectory(dirIn, dirOut, keystore, keyAlias, keystorePass, explicitBaseApk=None, zipalign=None, apksigner=None):
    if os.path.exists(dirOut):
        shutil.rmtree(dirOut)
    shutil.copytree(dirIn, dirOut)

    baseApkName = detectBaseApk(dirOut, explicitBaseApk)
    baseApkPath = os.path.join(dirOut, baseApkName)
    if not os.path.exists(baseApkPath):
        raise Exception("Base APK '{}' not found in '{}'".format(baseApkName, dirOut))

    print("Patching base APK '{}' ...".format(baseApkName))
    patchedBase = baseApkPath + ".patched.apk"
    patchApk(baseApkPath, patchedBase, keystore, keyAlias, keystorePass, zipalign=zipalign, apksigner=apksigner)
    os.replace(patchedBase, baseApkPath)

    for entry in sorted(os.listdir(dirOut)):
        if not entry.lower().endswith(".apk") or entry == baseApkName:
            continue
        srcApk = os.path.join(dirOut, entry)
        signedApk = srcApk + ".signed.apk"
        print("Re-signing split '{}' ...".format(entry))
        signApk(srcApk, signedApk, keystore, keyAlias, keystorePass, zipalign=zipalign, apksigner=apksigner)
        os.replace(signedApk, srcApk)

    return baseApkName

def patchXapk(fnIn, dirOut, keystore, keyAlias, keystorePass, explicitBaseApk=None, zipalign=None, apksigner=None):
    with tempfile.TemporaryDirectory(prefix="makeDebuggable_xapk_") as tmpDir:
        print("Extracting XAPK '{}' ...".format(fnIn))
        extractToDir(fnIn, tmpDir)
        return patchSplitDirectory(tmpDir, dirOut, keystore, keyAlias, keystorePass, explicitBaseApk, zipalign=zipalign, apksigner=apksigner)

def buildParser():
    parser = argparse.ArgumentParser(description="Patch Android APK/XAPK packages to set android:debuggable=true.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    parserXml = subparsers.add_parser("xml", help="Patch a binary AndroidManifest.xml file.")
    parserXml.add_argument("fileIn")
    parserXml.add_argument("fileOut", nargs="?")

    parserApk = subparsers.add_parser("apk", help="Patch and re-sign a single APK.")
    parserApk.add_argument("fileIn")
    parserApk.add_argument("fileOut")
    parserApk.add_argument("keystore")
    parserApk.add_argument("keyAlias")
    parserApk.add_argument("keystorePassword")
    parserApk.add_argument("--zipalign", dest="zipalign", default=None)
    parserApk.add_argument("--apksigner", dest="apksigner", default=None)

    parserSplit = subparsers.add_parser("split", help="Patch a split APK directory or extracted XAPK directory.")
    parserSplit.add_argument("dirIn")
    parserSplit.add_argument("dirOut")
    parserSplit.add_argument("keystore")
    parserSplit.add_argument("keyAlias")
    parserSplit.add_argument("keystorePassword")
    parserSplit.add_argument("--base-apk", dest="baseApk", default=None)
    parserSplit.add_argument("--zipalign", dest="zipalign", default=None)
    parserSplit.add_argument("--apksigner", dest="apksigner", default=None)

    parserXapk = subparsers.add_parser("xapk", help="Patch an XAPK file into a signed output directory.")
    parserXapk.add_argument("fileIn")
    parserXapk.add_argument("dirOut")
    parserXapk.add_argument("keystore")
    parserXapk.add_argument("keyAlias")
    parserXapk.add_argument("keystorePassword")
    parserXapk.add_argument("--base-apk", dest="baseApk", default=None)
    parserXapk.add_argument("--zipalign", dest="zipalign", default=None)
    parserXapk.add_argument("--apksigner", dest="apksigner", default=None)
...
if __name__ == "__main__":
    ...
    if args.command == "xml" and args.fileOut is None:
        with open(args.fileIn, "rb") as fIn:
            with BytesIO() as fOut:
                patchManifest(fIn, fOut)
    elif args.command == "xml":
        patchManifestByFilename(args.fileIn, args.fileOut)
    elif args.command == "apk":
        patchApk(args.fileIn, args.fileOut, args.keystore, args.keyAlias, args.keystorePassword, zipalign=args.zipalign, apksigner=args.apksigner)
    elif args.command == "split":
        baseApkName = patchSplitDirectory(args.dirIn, args.dirOut, args.keystore, args.keyAlias, args.keystorePassword, args.baseApk, zipalign=args.zipalign, apksigner=args.apksigner)
        installArgs = [
            '"{}"'.format(os.path.join(args.dirOut, name))
            for name in sorted(os.listdir(args.dirOut))
            if name.lower().endswith(".apk")
        ]
        print("Done. Signed split APKs are in '{}'.".format(args.dirOut))
        print("Install with {}".format("adb install-multiple " + " ".join(installArgs)))
        print("Base APK: {}".format(baseApkName))
    elif args.command == "xapk":
        baseApkName = patchXapk(args.fileIn, args.dirOut, args.keystore, args.keyAlias, args.keystorePassword, args.baseApk, zipalign=args.zipalign, apksigner=args.apksigner)
        installArgs = [
            '"{}"'.format(os.path.join(args.dirOut, name))
            for name in sorted(os.listdir(args.dirOut))
            if name.lower().endswith(".apk")
